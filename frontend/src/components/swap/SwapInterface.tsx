...
export function SwapInterface() {
  ...
  useEffect(() => {
    if (proofStep === "complete" && !isLoading) {
      const latestSwap = transactions.find(
  ...
  const getButtonState = () => {
    ...
    if (isPrivate) {
      if (availableNotes.length === 0) {
        return {
          disabled: true,
          text: "No notes available - Deposit first",
          reason: "no_notes",
        };
      }
      ...
      if (!selectedNote) {
        return {
          disabled: true,
          text: `Insufficient balance - Need ${amount} ${inputToken.symbol}`,
      ...
      if (selectedNote.index === undefined) {
        return {
          disabled: true,
          text: "Note not synced - Please wait",
          reason: "note_not_synced",
        };
      }
    }
    ...
    return {
      disabled: false,
      text: isPrivate ? "Private Swap" : "Swap",
      reason: "ready",
    };
  };
  ...
  const executeSwapAction = async () => {
    ...
    if (isPrivate) {
      if (!selectedNote) {
        alert(
          "No suitable note found. Please deposit tokens first or select a different note."
        );
        return;
      }
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-xs">
                  <p className="text-sm">
                    Private swaps use zero-knowledge proofs to hide transaction
                    amounts and maintain privacy. Your input note is replaced
                    with a new output note containing the swapped tokens.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        <CardContent className="space-y-4">
                ...
                >
                  ...
                  {availableNotes.map((note, idx) => (
                    <option key={idx} value={idx}>
                      Note #{idx + 1}: {note.amount.toString()} (Index:{" "}
                      {note.index ?? "N/A"})
                    </option>
                  ))}
                </select>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-yellow-300/80 mb-3">
                    Para hacer un swap privado, primero necesitas depositar
                    tokens. Esto creará una "nota privada" que podrás usar para
                    hacer swaps.
                  </p>
                  <Link
                    href="/portfolio"
          ...
          <div className="text-xs text-muted-foreground p-2 bg-stark-darker rounded border border-stark-gray/10 space-y-1">
            <div className="flex justify-between">
              <span>Amount:</span>
              <span>
                {amount || "empty"} → {amountNum} → {amountBigInt.toString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Valid Amount:</span>
              <span
                className={isValidAmount ? "text-green-400" : "text-red-400"}
              >
                {isValidAmount ? "Yes" : "No"}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            {isPrivate && (
              <>
                <div className="flex justify-between">
                  ...
                  <span>
                    {selectedNote
                      ? `Yes (Index: ${selectedNote.index ?? "N/A"})`
                      : "No"}
                  </span>
                </div>
                {selectedNote && (
            ...
            <div className="flex justify-between pt-1 border-t border-stark-gray/10">
              ...
              <span
                className={
                  buttonState.disabled ? "text-yellow-400" : "text-green-400"
                }
              >
                ...
                {proofStep === "computing_proof" && (
                  ...
                  >
                    <p className="text-sm text-blue-300">
                      💡 <strong>Generating ZK proof...</strong> This is the
                      most time-consuming step. The circuit is processing
                      ~22,000 constraints, which typically takes 2-3 minutes.
