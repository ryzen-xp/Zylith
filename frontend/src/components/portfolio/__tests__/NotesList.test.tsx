...
describe('NotesList - Core Logic', () => {
  ...
  it('should filter notes by token address', () => {
    ...
    const filtered = notes.filter(note => note.tokenAddress === selectedToken)
  ...
  it('should validate withdraw amount', () => {
    ...
    const decimals = 18
    const withdrawAmountBigInt = BigInt(Math.floor(parseFloat(withdrawAmount) * Math.pow(10, decimals)))
...
describe('NotesList - Note Management (TODO 4.4)', () => {
  ...
  it('should validate partial withdrawal strategy', () => {
    ...
    const isPartialWithdrawal = withdrawAmount < noteAmount && withdrawAmount > 0n
  ...
  it('should handle leaf index extraction from events', () => {
    ...
    const mockEvent = {
      from_address: '0x04b6a594dc9747caf1bd3d8933621366bbb7fbaefa1522174432611b577ae94d',
