...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let zylith_address = std::env::var("ZYLITH_CONTRACT").unwrap_or_else(|_| {
        "0x03a2134229b1938316f0db062a15c79a426df51805f57e8efcf306ef1e916aa6".to_string()
    });
    ...
    let account_address =
        std::env::var("ACCOUNT_ADDRESS").expect("ACCOUNT_ADDRESS environment variable must be set");
    ...
    println!(
        "   Check status at: https://sepolia.starkscan.co/tx/0x{:x}",
        transaction_hash
    );
